...
@Component
public class MISGamingRobiPOI {
	private static org.slf4j.Logger logger =LoggerFactory.getLogger(MISGamingRobiPOI.class); 

	private  void fillReport(Sheet worksheet, int startRowIndex, int startColIndex,List<MisGamingRobi> bean){
	...
	public  void genrateXLS(InputStream is, int startRowIndex, int startColIndex, HttpServletResponse response, List<MisGamingRobi> bean,String fromdate,String todate,String service_name) {
		try {
			...
			//Set Response Header
			response.setHeader("Content-Disposition", "inline; filename="+service_name+"_MIS_Robi.xlsx");
