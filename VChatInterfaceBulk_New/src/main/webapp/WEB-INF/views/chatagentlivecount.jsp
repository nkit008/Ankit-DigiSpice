...
<link rel="stylesheet" type="text/css"
	href="resources/css/buttons.dataTables.min.css">
...
<script type="text/javascript">
	function getVal(a, b) {
		var data = "";
		data = 'hub=' + encodeURIComponent(a) + '&partnerName='+encodeURIComponent(b);
		window.location.replace('getPartnerDetails?'+data);
		return false;
	}
</script>
...
<body>
						...
						<tbody>
							<c:forEach var="listValue" items="${objZoneResp}">
								<tr>
									...
									<c:set var="total"
										value="${listValue.loginId + listValue.logout + listValue.aLogout + listValue.bLogout + listValue.fLogout + listValue.sLogout}" />
									<c:choose>
		...
		<div class="card bgcrd">
			<div class="card-body">
				<div class="x_content">
					...
					<table id="datatable1" class="display dtb"
						...
						<tbody>
							<c:forEach var="listValue" items="${objCircleResp}">
								<tr>
									<td><a
										href="circlewiseAgentdetails?hub=${hubName}&circle=${listValue.circle}">${listValue.circle}</a></td>

									<c:set var="total" value="${0}" />
									<c:set var="total"
										value="${listValue.login + listValue.logout + listValue.aLogout + listValue.bLogout + listValue.fLogout + listValue.sLogout}" />
									<c:choose>
										<c:when test="${empty total}">
											<td><b>0</b></td>
										</c:when>
	...
	<div class="" style="width: 100%;">
		<div class="card bgcrd">
			<div class="card-body">
				<div class="x_content">
					<h4 class="page-title" align="center" style="color: #FFF;">
						Partner Wise</h4>
					<table id="datatable2" class="display nowrap"
						style="width: 100%; text-align: center; font-size: 11px;">
						<thead>
							<tr class="trb">
								<th>Partner Name</th>
						...
						<tbody>
							<c:forEach var="listValue" items="${objPartnerResp}">
								<tr>
									...
									<td><a href="" onclick="return getVal('${listValue.hub}','${listValue.partnerName}')"> ${listValue.partnerName} </a></td>
	...
	<script src="resources/js/dataTables.buttons.min.js"
